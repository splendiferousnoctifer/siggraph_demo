...
router.beforeEach((to, from, next) => {
  if (to.name !== 'PlaybackPage' && ((to.name === 'FinalPage' || to.meta.order > 1) && !from.name)) {
    // Redirect to home if accessing final page or any other page directly, except for playback page
    next({ name: 'Home' });
  } else {
    next();
  }
